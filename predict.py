...
class Predict(Base):
    ...
    def create_predictions(self):
        for _ in range(1000000):
            try:
                ...
                filename = self.test_generator.get_name(_)[0]
                extents = self.test_generator.get_extent(_)[0]
                pr_gradient = self.test_model.predict(image)[0]
    ...
    def evaluate_watershed(self):
        ...
        confusion_matrices.to_csv(
            f'output/evaluation/confusion_matrices_{self.source}_{self.area}_from_scratch_{self.from_scratch}.csv')
    ...
    def evaluate_polis(self):
        ...
        polis_gdf_all.to_csv(f'output/evaluation/polis_{self.source}_{self.area}_from_scratch_{self.from_scratch}.csv')
